<script lang="ts">
  ...
  const projects = [
    ...
    // },
    {
      ...
      external: true,
      tags: [
        { title: "Made at *Space", color: "blue" },
        { title: "Open Source", color: "green" },
      ],
    },
    {
      name: "Spacetime Clock",
      description:
        "A date and time clock where the current day and the current time are represented as a degree of a circle (0-359) with the start date being the last summer solstice and the start time being solar noon.",
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    {#each projects as project}
      <Card class="!bg-opacity-50 relative">
        ...
        <p
          class="mb-5 font-light text-gray-700 dark:text-gray-300 leading-tight text-left"
        >
          {project.description}
        </p>
        {#if project.tags}
        ...
        {/if}
        <div class="text-right absolute bottom-3 right-3">
          <Button
            href={project.url}
